...
const GoogleButton: React.FC<GoogleButtonProps> = ({ onClick }) => {
  ...
  return (
    <button
      className="flex w-full justify-center gap-3 custom-rounded border-2 border-black bg-white px-4 py-2 text-sm font-bold drop-shadow-md hover:bg-gray-50"
      onClick={handleClick}
    >
      <GoogleLogo />
      <div className="text-black text-xl font-normal font-raleway">Google</div>
    </button>
  );
};
